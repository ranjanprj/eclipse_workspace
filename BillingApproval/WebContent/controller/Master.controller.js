/*global history */
sap.ui.define([
	"com/slb/cus/controller/BaseController",
	...
	"sap/ui/Device",
	"com/slb/cus/model/formatter",
	"com/slb/cus/model/grouper"
], function(BaseController, JSONModel, Filter, FilterOperator, Sorter, GroupHeaderListItem, Device, formatter, grouper) {
	"use strict";

	return BaseController.extend("com.slb.cus.controller.Master", {
		...
		onAfterRendering : function(){
			 //this._onOpenIntialFilterDialog();
		},
		/* =========================================================== */
		/* event handlers                                              */
		/* =========================================================== */
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("com.slb.cus.view.ViewSettingsDialog", this);
		...
		_onOpenIntialFilterDialog: function() {
			if (!this._oViewIntialFilterDialog) {
				this._oViewIntialFilterDialog = sap.ui.xmlfragment("com.slb.cus.view.InitialFilterDialog", this);
				this.getView().addDependent(this._oViewIntialFilterDialog);
				// forward compact/cozy style into Dialog
				this._oViewIntialFilterDialog.addStyleClass(this.getOwnerComponent().getContentDensityClass());
			}
			this._oViewIntialFilterDialog.open();
		},
			...
			aFilterItems.forEach(function(oItem) {
				...
				switch (oItem.getParent().getKey()) {
					...
					case "Filter2":
						...
						break;
					case "SALES_ORG_KEY":
						aFilters.push(new Filter("Vkorg", FilterOperator.Contains, oItem.getKey()));
						break;
					case "DIST_CHANNEL_KEY":
						aFilters.push(new Filter("Vtweg", FilterOperator.Contains, oItem.getKey()));
						break;
					case "DIV_KEY":
						aFilters.push(new Filter("Spart", FilterOperator.Contains, oItem.getKey()));
						break;
					case "DOC_TYPE_KEY":
						aFilters.push(new Filter("DocTypeId", FilterOperator.Contains, oItem.getKey()));
						break;
					case "JOB_PRJ_KEY":
						aFilters.push(new Filter("Job", FilterOperator.Contains, oItem.getKey()));
						break;
					case "CUST_SOLD_TO_KEY":
						aFilters.push(new Filter("CustomerId", FilterOperator.Contains, oItem.getKey()));
						break;
					default:
						break;
	...
	/**
		 ...
		 * @public
		 */
		onConfirInitialFilterDialog: function(oEvent) {
			var aFilterItems = oEvent.getParameters().filterItems,
				aFilters = [],
				aCaptions = [];
