...
sap.ui.define([
	...
	"com/slb/cus/operationalapproval/model/grouper"
], function(BaseController, JSONModel, Filter, FilterOperator, Sorter, GroupHeaderListItem, Device, formatter, grouper) {
	"use strict";

	return BaseController.extend("com.slb.cus.operationalapproval.controller.Master", {
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("OrderNo", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("com.slb.cus.operationalapproval.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function(oEvent) {
			...
			aFilterItems.forEach(function(oItem) {
				switch (oItem.getParent().getKey()) {
				 case "SALES_ORG_KEY":
		                        aFilters.push(new Filter("SalesOrg", FilterOperator.EQ, oItem.getKey()));
		                        break;
		                    case "DIST_CHANNEL_KEY":
		                        aFilters.push(new Filter("DisChannel", FilterOperator.EQ, oItem.getKey()));
		                        break;
		                    case "DIV_KEY":
		                        aFilters.push(new Filter("Division", FilterOperator.EQ, oItem.getKey()));
		                        break;
		                    case "DOC_TYPE_KEY":
		                        aFilters.push(new Filter("OrderType", FilterOperator.EQ, oItem.getKey()));
		                        break;
		                    case "JOB_PRJ_KEY":
		                        aFilters.push(new Filter("JobId", FilterOperator.EQ, oItem.getKey()));
		                        break;
		                    case "CUST_SOLD_TO_KEY":
		                        aFilters.push(new Filter("SoldToParty", FilterOperator.EQ, oItem.getKey()));
		                        break;
		                    case "WELL_KEY":
		                        aFilters.push(new Filter("WellId", FilterOperator.EQ, oItem.getKey()));
		                        break;
		                    case "RIG_KEY":
		                        aFilters.push(new Filter("RigId", FilterOperator.EQ, oItem.getKey()));
		                        break;
		                    case "SEG_KEY":
		                        aFilters.push(new Filter("SegmentId", FilterOperator.EQ, oItem.getKey()));
		                        break;
		                    case "SUB_SEG_KEY":
		                        aFilters.push(new Filter("SubSegmentId", FilterOperator.EQ, oItem.getKey()));
		                        break;
		                    default:
		                        break;
				}
		...
		_onMasterMatched: function() {
			this.getOwnerComponent().oListSelector.oWhenListLoadingIsDone.then(
				function(mParams) {
					...
					var sObjectId = mParams.firstListitem.getBindingContext().getProperty("OrderNo");
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				objectId: oItem.getBindingContext().getProperty("OrderNo")
			}, bReplace);
		},
